...
- **Multi-Source Tag Fetching:**
  ...
  - **Reverse Image Search:** Uses SauceNao to discover sources when direct matching fails. Consider upgrading your account with them if you arent patient.
  - **AI Tagging:** Local ONNX-based tagger runs only when no online sources provide metadata
  - **Flexible Source Switching:** View tags from different sources or merge them all together

- **AI Rating Inference:** Automatic content rating detection (General, Sensitive, Questionable, Explicit) with confidence scores

### 🔍 Search System

- **Tag Search:** Autocomplete with category-specific filtering (`character:name`, `artist:name`, `copyright:series`)
...
- **Similarity Search:** Weighted similarity matching that prioritizes rare tags and important categories
...
- **Debug Panel:** Tools for database maintenance, optimization, and bulk operations
- **Health Checks:** Automatic startup health checks with self-repair
...
git clone https://github.com/kooten111/ChibiBooru
cd ChibiBooru

# Create a virtual environment (3.12 is tested)
python3 -m venv venv

# Run (handles setup wizard, dependencies, and starts the server)
./start_booru.sh
...
- Python 3.10–3.12 (required by onnxruntime)
...
2. **Create a virtual environment** (optional - `start_booru.sh` will create it if missing):
   ```bash
   python3 -m venv venv
   ```
...
3. **Run the start script:**
   ...
   - Run the setup wizard (on first run) to configure secrets and download AI models
   - Start the application with uvicorn and the monitor service
...
**If you need to rebuild your virtual environment:**
```bash
./start_booru.sh --rebuild-venv
```

**For development mode (single worker, no monitor):**
```bash
...
If you prefer to manually download the tagger model, place these files in `models/Tagger/`:
...
SYSTEM_API_SECRET="your-system-api-secret"
...
- `SYSTEM_API_SECRET` - System control API secret (for administrative operations)
...
> [!CAUTION]
> - **Always change** `APP_PASSWORD`, `SECRET_KEY`, and `SYSTEM_API_SECRET` before deployment
...
4. **Navigate to System page** (`/system` in the navigation bar) and enter your `SYSTEM_API_SECRET`
...
- **Web Workers (uvicorn):** Handle HTTP requests with multiple workers for scalability
...
- **[Services](docs/SERVICES.md)** - Core application logic and all services documented
...
1. Review the architecture documentation:
   ...
   - [Services Documentation](docs/SERVICES.md) for application logic
...
- **Services Layer** (`services/`) - Core application logic (16+ top-level services with sub-packages)
