...
| Character | `character_service.py` | AI-based character inference |
...
### Purpose
AI-based character identification using tag-based machine learning trained on booru data.

### Overview
The character inference system learns from character tags in booru-sourced images to automatically predict which characters appear in locally-tagged images. It uses the same log-likelihood ratio algorithm as the rating service but adapted for multi-label character prediction.

### Data Sources
- **Training**: Images from `danbooru`, `e621`, `gelbooru`, `yandere` with character tags
- **Inference**: Locally-tagged images (from `local_tagger`) without character tags

### Functions

#### `train_model() -> Dict`

Train the character inference model from booru-tagged images.

**Process**:
1. Extract all images with character tags from trusted booru sources
2. Calculate tag weights for each character (log-likelihood ratios)
3. Calculate tag pair weights for contextual information
4. Store weights in separate model database (`character_model.db`)
5. Update model metadata
...
```python
{
    ...
    "unique_characters": 250,
    "unique_tags": 3000,
    "unique_pairs": 8000,
    "tag_weights_count": 15000,
    "pair_weights_count": 20000,
    "duration_seconds": 45.2
}
```

**Side Effects**:
- Populates `character_tag_weights` table
- Populates `character_tag_pair_weights` table
- Updates `character_model_metadata`
...
#### `predict_characters(image_tags: List[str]) -> List[Tuple[str, float]]`
...
| Name | Type | Description |
|------|------|-------------|
| `image_tags` | `List[str]` | List of tag names (excluding character tags) |
...
```
Sorted by confidence descending, filtered by `min_confidence` threshold, limited by `max_predictions`.

**Algorithm**:
1. Load tag weights and pair weights from model database
2. Calculate raw scores for each known character:
   - Sum individual tag weights
   - Sum tag pair weights (with `pair_weight_multiplier`)
3. Convert scores to probabilities using softmax
4. Filter by `min_confidence` threshold
5. Return top N predictions (up to `max_predictions`)

---

#### `infer_character_for_image(image_id: int) -> Dict`

Run inference on a single image and apply predicted character tags.
...
**Side Effects**:
- Adds character tags to image with `source='ai_inference'`
- Triggers cache reload

---

#### `infer_all_untagged_images() -> Dict`

Run inference on all locally-tagged images without character tags.
...
```python
{
    "processed": 1500,
    "tagged": 1200,
    "skipped_low_confidence": 300,
    "characters_added": 1800,
    "by_character": {
        "hatsune_miku": 450,
        "kagamine_rin": 250,
        ...
...
**Side Effects**:
- Adds character tags to images with `source='ai_inference'`

---

#### `clear_ai_inferred_characters() -> int`
...
**Side Effects**:
- Deletes all `image_tags` where `source='ai_inference'` and `category='character'`
...
```python
{
    "model_trained": True,
    "metadata": {
        "last_trained": "2024-01-08T12:34:56",
        "training_sample_count": "5000",
        "unique_characters": "250",
        "unique_tags_used": "3000",
        "unique_pairs_used": "8000"
    },
    "config": {
    ...
    "character_distribution": {
        "hatsune_miku": {"total": 500, "ai": 200, "user": 50, "original": 250},
        ...
    },
    "untagged_images": 1500
}
```

---

#### `get_all_known_characters() -> List[Dict]`
...
```python
[
    {"name": "hatsune_miku", "sample_count": 450, "total": 500, "ai": 200, "user": 50, "original": 250},
    {"name": "kagamine_rin", "sample_count": 250, "total": 300, "ai": 150, "user": 30, "original": 120},
    ...
]
```
...
Configuration stored in `character_model.db` table `character_inference_config`:

| Key | Default | Description |
|-----|---------|-------------|
| `min_character_samples` | 10 | Minimum training images to include character |
...
| `min_pair_cooccurrence` | 5 | Minimum tag pair co-occurrence for training |
| `min_tag_frequency` | 10 | Minimum tag frequency for pair calculation |
| `max_pair_count` | 10000 | Maximum tag pairs to store |
| `pruning_threshold` | 0.0 | Minimum weight magnitude to store (0 = disabled) |

---

### Model Database

Stored in separate SQLite database: `character_model.db`

**Tables**:
- `character_inference_config` - Configuration values
- `tags` - Tag name to ID mapping
- `characters` - Character name to ID mapping
- `character_tag_weights` - Tag → Character weights
- `character_tag_pair_weights` - Tag Pair → Character weights
- `character_model_metadata` - Training timestamps and stats

**Benefits of Separate DB**:
- Can be distributed as pre-trained model
- Isolated from main database changes
